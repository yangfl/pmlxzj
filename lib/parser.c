...
int Plzj_print_info (const struct Plzj *pl, FILE *out, bool in_section) {
  ...
  if (in_section) {
    ...
    ret += fprintf(out, "Section caption: %s\n",
...
int Plzj_init (
  ...
  return_if_fail (reset_offset != -1) ERR_SYS(ftello);
...
void PlzjFile_destroy (struct PlzjFile *pf) {
  for (uint32_t i = 0; i < pf->sections_cnt; i++) {
...
int PlzjFile_init (struct PlzjFile *pf, FILE *file) {
  ...
  ret = read_at(file, -sizeof(magic), SEEK_END, &magic, sizeof(magic));
  goto_if_fail (ret == 0) fail;
  ...
  if (!pf->extended) {
    ...
    for (uint32_t i = 0; i < pf->sections_cnt; i++) {
      ret = read_at(
        ...
        sizeof(struct PlzjLxeSection) * (pf->sections_cnt - i),

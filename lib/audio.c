...
static int _Plzj_extract_audio_wav_zlib (
  ...
  for (uint32_t i = 0; i < le32toh(audio.chunks_cnt); i++) {
    size_t chunk_size;
    size_t out_size;
    static_assert(4096 * 32 >= PLZJ_AUDIO_WAV_ZLIB_MAX_CHUNK_SIZE);
    ret = write_lpe_uncompress(out, file, &chunk_size, 4096 * 32, &out_size);
    goto_if_fail (ret == 0) fail;

    if_fail (out_size <= PLZJ_AUDIO_WAV_ZLIB_MAX_CHUNK_SIZE) {
      sc_warning(
        "WAV uncompressed chunk too large (%zu"
        " > %u), potentially corrupted\n",
        out_size, PLZJ_AUDIO_WAV_ZLIB_MAX_CHUNK_SIZE);
    }
    sc_debug("WAV zlib chunk %" PRIu32 ": size %zu, uncompressed %zu\n",
...
static int _Plzj_extract_audio_truespeech (
  ...
  uint32_t size = 12 + 8 + sizeof(audio.ts_spec) + 32 + 8 + data_size;
  sc_debug("TrueSpeech file size: %" PRIu32 "\n", size);
  ...
  unsigned char header[20] = "RIFF\0\0\0\0WAVEfmt ";
  ...
  *(uint32_t *) (header + 16) = htole32(sizeof(audio.ts_spec) + 32);
  ...
  if_fail (fwrite(&audio.ts_spec, sizeof(audio.ts_spec), 1, out) == 1) {
    ret = ERR_SYS(fwrite);
    goto fail;
  }
  unsigned char formatex[32] = "\1\0\xf0";
  if_fail (fwrite(&formatex, sizeof(formatex), 1, out) == 1) {
    ret = ERR_SYS(fwrite);
    goto fail;
  }

  unsigned char datachunk[8] = "data";
  *(uint32_t *) (datachunk + 4) = htole32(data_size);
  if_fail (fwrite(&datachunk, sizeof(datachunk), 1, out) == 1) {
    ret = ERR_SYS(fwrite);
    goto fail;
  }
...
static void PlzjAudioAAC_destroy (struct PlzjAudioAAC *aac) {
  free(aac->config);
...
static int PlzjAudioAAC_init (struct PlzjAudioAAC *aac, FILE *in) {
  ...
  return_if_fail (fread(&h_aac, sizeof(h_aac), 1, in) == 1) ERR_SYS(fread);
  ...
  return_if_fail (aac->config != NULL) ERR_SYS(malloc);
  ...
  for (uint32_t i = 0; i < aac->stream_data_sizes_cnt; i++) {
    if_fail (aac->stream_data_sizes[i] <= PLZJ_ADTS_MAX_DATA_LEN) {
      ret = ERR_FMT(
        PL_EFORMAT, "AAC stream too long (%" PRIu32 " > %u)",
  ...
  if_fail (data_size == aac->data_size) {
    sc_warning("AAC data size mismatched, expected %zu, got %" PRIu32 "\n",

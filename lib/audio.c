...
static int _Pmlxzj_extract_audio_wav_zlib (
  ...
  int ret = 0;
  for (uint_fast32_t i = 0; i < le32toh(audio.segments_cnt); i++) {
...
static int _Pmlxzj_extract_audio_aac (
  ...
  for (uint_fast32_t i = 0; i <= le32toh(audio.format_len); i++) {
    uint8_t format;
  ...
  uint_fast32_t segment_sizes_cnt =
  ...
  for (uint_fast32_t i = 0; i < segment_sizes_cnt; i++) {
...
int Pmlxzj_extract_audio (const struct Pmlxzj *pl, const char *dir) {
  return_if_fail (pl->audio_offset != 0) 0;

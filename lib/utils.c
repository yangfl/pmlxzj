...
void *ptrarray_new (void *arrp, size_t *lenp, size_t elmsize) {
  size_t newlen = *lenp + 1;
  size_t part = 1ull << (sizeof(size_t) * 8 - stdc_leading_zeros(newlen) - 1);
  if (part > 4096) {
    part = 4096;
  }
  newlen = (newlen + part - 1) & ~(part - 1);

  void **newarr = realloc(*(void ***) arrp, sizeof(void *) * newlen);
  if_fail (newarr != NULL) {
...
int array_new (
    ...
    void *userdata) {
  void *newarr = realloc(*(void **) arrp, elmsize * (*lenp + 1));
...
static int write_lpe_func (FILE *dst, FILE *src, stream_fn_t func) {
  int ret;

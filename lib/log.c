...
SC_WEAK int sc_print_err_f (
    ...
    const char *bt_indent) {
  return ScException_print(&sc_exc, printer, data, indent, bt_indent);
}


SC_WEAK int sc_print_err (
    FILE *out, const char *indent, const char *bt_indent) {
  return sc_print_err_f((sc_print_fn_t) fprintf, out, indent, bt_indent);
}

...
SC_WEAK int sc_register_strerror (unsigned int domain, sc_strerror_fn_t strfn) {
...
__always_inline
static inline int _save_err (unsigned long domain, int code, const char *fn) {
...
SC_WEAK int sc_set_err (unsigned long domain, int code, const char *fn) {
  return _save_err(domain, code, fn);
}


SC_WEAK int sc_set_errno (const char *fn) {
  return _save_err(0, errno, fn);
}

...
__wur __attribute_pure__ __nonnull() __attr_access((__read_only__, 1))
static inline uint_fast32_t PmlxzjLxePacket_frame_no (const void *packet) {
  int32_t frame_no = le32toh(*(const int32_t *) packet);
  return frame_no > 0 ? frame_no : 1 - frame_no;
}

__wur __attribute_pure__ __nonnull() __attr_access((__read_only__, 1))
static inline uint_fast32_t PmlxzjLxePacket_data_size (const void *packet) {

...
DLL_PUBLIC __THROW __nonnull() __attr_access((__read_only__, 1, 2))
int sc_print_backtrace (void *const *frames, int size, FILE *out);
...
int sc_register_strerror (unsigned long domain, sc_strerror_fn_t strfn);
...
DLL_PUBLIC __nonnull((1))
int sc_print_err_f (sc_print_fn_t printer, void *data);
DLL_PUBLIC __THROW __nonnull()
int sc_print_err (FILE *out);

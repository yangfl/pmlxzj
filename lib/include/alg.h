...
__attribute_artificial__ __nonnull() __attr_access((__write_only__, 1, 2))
__attr_access((__read_only__, 3, 4))
/**
 * @brief Convert pmlxzj character encoding to local encoding, insuring null
 * termination.
 *
 ...
 *   @p dst .
 */
static inline size_t pmlxzj_iconv_dec (
    char *dst, size_t dstsize, const char *src, size_t srclen) {
  return pmlxzj_iconv("", "GBK", dst, dstsize, src, srclen);
}

__attribute_artificial__ __nonnull() __attr_access((__write_only__, 1, 2))
__attr_access((__read_only__, 3, 4))
/**
 * @brief Convert local character encoding to pmlxzj encoding, insuring null
 * termination.
 *
 * @note Be sure to call <tt>setlocale(LC_ALL, "")</tt> before using this
 * f  unction.
 ...
 *   @p dst .
 */
static inline size_t pmlxzj_iconv_enc (
    char *dst, size_t dstsize, const char *src, size_t srclen) {
  return pmlxzj_iconv("GBK", "", dst, dstsize, src, srclen);
}

DLL_PUBLIC __THROW __attribute_malloc__ __nonnull()
__attr_access((__read_only__, 1))
char *pmlxzj_iconv_enc_new (const char *src, size_t srclen);
 ...
 */
static inline unsigned long pmlxzj_password_cksum(const void *password) {
...
/**
 * @brief Convert password to pmlxzj encryption key.
 *
 ...
 */
static inline void pmlxzj_key_enc (void *key, const void *password) {
...
__attr_access((__read_only__, 2))
/**
 * @brief Encode / decode pmlxzj info text.
 *
 * @c lxefileplay::infotextjm()
 *
 * @param[out] dst Output buffer. Must be at least 40 bytes long.
 * @param src Input buffer. Must be at least 40 bytes long.
 * @param len Length of @p dst and @p src .
 */
static inline void pmlxzj_infotext_encdec (void *dst, const void *src) {
...
DLL_PUBLIC __THROW __nonnull() __attr_access((__write_only__, 1, 2))
__attr_access((__read_only__, 3))
size_t pmlxzj_infotext_dec_iconv (char *dst, size_t dstsize, const char *src);
DLL_PUBLIC __THROW __nonnull() __attr_access((__write_only__, 1))
__attr_access((__read_only__, 2, 3))
size_t pmlxzj_infotext_enc_iconv (char *dst, const char *src, size_t srclen);


__attribute_artificial__ __wur __attribute_pure__ __nonnull()
__attr_access((__read_only__, 1))
static inline long pmlxzj_regcode_calc (const char *regcode1) {
  long code1 = -100;
...
__attribute_artificial__ __wur __attribute_pure__ __nonnull()
__attr_access((__read_only__, 1))
static inline long pmlxzj_regcode_dec (const char *regcode2) {
  long code2 = 0;
...
static inline bool pmlxzj_regcode_check (
    const char *regcode1, const char *regcode2) {
  return pmlxzj_regcode_calc(regcode1) == pmlxzj_regcode_dec(regcode2);
}
 ...
 *
 * @note Do not use @c pmlxzj_regcode_enc() to check against @p regcode2 . Check
 *   @c pmlxzj_regcode_dec(regcode2) against @p code2 instead.
 *
 * @param[out] regcode2 Output buffer. Must be able to hold an integer (at least
 *   11 bytes long).
 * @param code2 Code 2. Ideally @c pmlxzj_regcode_calc(regcode1) .
 * @return @c strlen(regcode2) .
 */
unsigned int pmlxzj_regcode_enc (char *regcode2, long code2);


__attribute_artificial__ __nonnull((1)) __attr_access((__read_write__, 1, 2))
__attr_access((__read_only__, 3))
static inline bool pmlxzj_image_encdec (
    void *buf, size_t len, const void *key) {

...
int PlzjCursor_print (
  ...
  if (cursor->seg.size) {
    ret += fprintf(out, "    Stream length: 0x%zx (%zu)\n",
...
static int PlzjCanvas_init (
  ...
  return_if_fail (canvas->pixels != NULL) ERR_SYS(malloc);
  canvas->width = width;
...
static int PlzjEncoder_write_apng (
  ...
  ret = plzj_png_write_info(
    png_ptr, encoder->canvas_last.width, encoder->canvas_last.height, pl);
  goto_if_fail (ret == 0) fail;
...
static int PlzjEncoder_stop (
  ...
  const struct ScException *exc;
  int ret = ThreadPool_get_err(&encoder->pool, &exc);
  if_fail (ret == 0) {
    sc_exc = *exc;
  }
  return ret;
...
int PlzjVideo_write_apng (
  ...
  for (size_t i = 0; i < video->frames_cnt; i++) {
    if (i % 32 == 0 || i + 1 >= video->frames_cnt) {
      sc_notice(

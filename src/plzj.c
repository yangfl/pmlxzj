...
#include "lib/include/platform/portable_endian.h"
    ...
    switch (option) {
      ...
      case 'n':
        if_fail (argtol(optarg, &options->frames_limit, 0, INT32_MAX) == 0) {
          fputs("error: number of frame not a non-negative integer\n", stderr);
          return -2;
        }
        break;
      case 'c':
      ...
      case 't':
        ...
        if_fail (argtol(optarg, &options->nproc, 0, INT_MAX) == 0) {
          fputs("error: number of thread not a non-negative integer\n", stderr);
          return -2;
        }
#endif
        break;
...
static int do_extract (
  ...
  if (options->extract_audio) {
    if (pl->audio_offset == -1) {
      ...
      int res = Plzj_extract_audio(pl, dir);
      if_fail (res >= 0) {
        what = "audio";
        goto fail;
      }
...
static int do_modify (
  ...
  FILE *file = fopen(options->output_path, "w+b");
  if_fail (file != NULL) {
    ret = ERR_SYS(fopen);
    what = "open";
    goto fail_open;

...
Default output directory is <video>.extracted .\n\
  ...
  -p, --fps <rate>      target to <rate> fps as much as possible by video frame\n\
                        interpolation, smooth cursor movement\n\
                        (ignored if '-r' specified) (default: 30)\n\
  -r, --raw             do not draw cursor in the output video file\n\
...
static int get_options (struct PmlxzjOptions *options, int argc, char **argv) {
  *options = (struct PmlxzjOptions) {
  ...
  for (int option; (option = getopt_long(
       argc, argv, "-eap:rmn:t:uk:fvdh", longopts, NULL)) != -1;) {
    switch (option) {
      case 1:
      ...
      case 'p':
        if_fail (argtof(optarg, &options->fps, 0, 480) == 0) {
          fputs("error: fps not a positive number\n", stderr);
          return -2;
        }
        break;
      case 'r':
